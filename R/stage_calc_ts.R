...
#' @examples
#' \dontrun{
#' files <- stage_calc_ts(sites="nwis_08062500", var="suntime", src="calcLon", verbose=TRUE)
#' str(read_ts(files))
#' files <- stage_calc_ts(sites="nwis_01484680", var="dosat", src="calcGG", verbose=TRUE)
#' }
#' @export
    ...
    ts_calc <- tryCatch({
      switch(
        ...
        'suntime_calcLon' = {
          ...
          utctime <- read_ts(download_ts("doobs_nwis", site, on_local_exists="replace"))$DateTime # could be using merged data, not just doobs...or maybe doobs is good enough
          ...
          data.frame(
            ...
            mutate(suntime=u(suntime, tz(suntime))) %>%
            as.data.frame() %>% u()
        },
        'light_calcLat' = {
